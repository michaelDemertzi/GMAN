...
#read in CSV
with open('allBillsStateZip.csv','rb') as f:
    reader = csv.reader(f)
    i = 0
    rowNum=0
    failed = 0 #failed accrues with invalid data such as NAs
    suceed = 0
    ...
    for row in reader:
        if rowNum == 0:
            ...
            try:            
                out[row[16]] = out.get(row[16],0) + int(re.search('[0-9]+',row[6]).group(0))
                suceed += 1
            except:
                failed += 1
...
#write dict to csv
with open('aggregateStates.csv', 'wb') as f:
    writer = csv.writer(f)
